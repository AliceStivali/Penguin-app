...
export function News() {
  ...
  return (
    <div className="flex-col flex-wrap justify-center items-space-between; news-list-style">
      ...
      {data && (
        <div>
          <p style={{ paddingLeft: "5rem", paddingTop: "0.7rem" }}>
            What's new in {data.user_input.q}
          </p>
      ...
      <div className="news-card-buttons">
        <div className="news-arrow-btn-container">
          {i !== 0 && (
            <button className="arrow-button" onClick={changeIndexBackwards}>
              {" "}
              ◀{" "}
            </button>
          )}
          <button className="arrow-button" onClick={changeIndexForward}>
            {" "}
            ▶{" "}
          </button>
        </div>
